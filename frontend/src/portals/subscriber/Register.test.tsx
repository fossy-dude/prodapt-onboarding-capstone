...
vi.mock('../../lib/api', () => ({
  ERROR_CODES: { DUPLICATE_MSISDN: 'DUPLICATE_MSISDN', VALIDATION_ERROR: 'VALIDATION_ERROR' },
...
async function fillStep1(user: ReturnType<typeof userEvent.setup>, msisdn = '9876543210') {
  ...
  await user.type(screen.getByLabelText('Alternate mobile (for OTP)'), '9123456780');
}
...
describe('Register', () => {
  ...
  it('navigates Step 1 → Step 2 → back', async () => {
    ...
    expect(screen.getByText('TRAI Customer Acquisition Form')).toBeInTheDocument();
  ...
  it('surfaces a 409 DUPLICATE_MSISDN as an inline Step 1 error', async () => {
    vi.mocked(registerSubscriber).mockRejectedValueOnce({ apiCode: 'DUPLICATE_MSISDN' });
    const user = userEvent.setup();
    renderRegister();
    await fillStep1(user);
    await user.click(screen.getByRole('button', { name: 'Next' }));
  ...
  it('shows the Registration ID on Step 3 on success', async () => {
    vi.mocked(registerSubscriber).mockResolvedValueOnce({
      data: { registration_id: 'REG-20260620-deadbabe', status: 'REGISTRATION_COMPLETE' },
    ...
    await waitFor(() => expect(screen.getByLabelText('Registration ID')).toHaveTextContent('REG-20260620-deadbabe'));
