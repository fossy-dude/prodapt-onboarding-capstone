...
describe("Subscriber profile page", () => {
  ...
  it("renders the pending Badge variant for kyc_status=pending (AC #2)", async () => {
    ...
    expect(
      screen.queryByRole("link", { name: /re-submit kyc/i }),
  ...
  it("renders the rejected Badge, rejection reason, and a re-submit link (AC #2, #5)", async () => {
    ...
    const resubmit = screen.getByRole("link", {
      name: /re-submit kyc documents/i,
    });
  ...
  it("submits the edit form → PATCH then refetches the profile (AC #3, #4)", async () => {
    ...
    vi.mocked(updateProfile).mockResolvedValue(
      makeProfile("verified", { email: "new@example.com" }),
    );
    renderProfile();
    await waitFor(() =>
      expect(screen.getByText("Priya Sharma")).toBeInTheDocument(),
    ...
    await waitFor(() => {
      expect(vi.mocked(getProfile).mock.calls.length).toBeGreaterThan(
        initialCalls,
      );
    });
