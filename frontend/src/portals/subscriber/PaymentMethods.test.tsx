...
const mockSetDefaultPaymentMethod = setDefaultPaymentMethod as ReturnType<typeof vi.fn>;
...
describe('PaymentMethods', () => {
  ...
  describe('loading and error states', () => {
    it('shows loading state while fetching payment methods', () => {
      ...
      expect(screen.getByText('Loading your saved payment methods...')).toBeInTheDocument();
    });
    ...
    it('shows empty state when no payment methods exist', async () => {
      mockGetPaymentMethods.mockResolvedValue({ data: [], meta: { trace_id: 'abc', timestamp: '2025-01-01' } });
      renderWithQueryClient(<PaymentMethods />);

      await waitFor(() => {
        expect(screen.getByText('No payment methods saved yet.')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('form validation', () => {
    it('disables add button when identifier is empty', async () => {
      ...
      await waitFor(() => {
        const submitButton = screen.getAllByRole('button').find(btn => btn.textContent === 'Add Payment Method');
    ...
    it('enables add button when identifier is filled', async () => {
      ...
      await waitFor(() => expect(screen.getByLabelText(/Card Number/i)).toBeInTheDocument());
